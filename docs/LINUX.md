...
3. A benchmark-flagged production host launches under a private D-Bus session
   ...
   performs and restores a native clipboard round trip, streams 2 MiB through a
   same-executable PTY child, performs five live resizes, closes under output,
   and records the app, PTY child, and WebKitGTK helper process tree.
